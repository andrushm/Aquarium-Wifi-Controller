...
PumpOut pumpOut(D6);
LightOut lampOut(D7);
Scheduler scheduler(pumpOut);
Scheduler schedulerLight(lampOut);
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if (strcmp(topic, timeOnTopic) == 0)
  {
    setTime.hour = payloadStr.substring(0, 1).toInt();
    setTime.mins = payloadStr.substring(2, 3).toInt();
    schedulerLight.setTimeOn(setTime);
  }
  ...
  if (strcmp(topic, timeOffTopic) == 0)
  {
    setTime.hour = payloadStr.substring(0, 1).toInt();
    setTime.mins = payloadStr.substring(2, 3).toInt();
    schedulerLight.setTimeOff(setTime);
  }
...
void setup()
{
  ...
  scheduler.onMinuteTick([]() {
    Serial.println(scheduler.now);
    client.publish("time", ctime(&scheduler.now));
  });
  scheduler.onCheckTimeCallback([](boolean result) {
    if (result)
    {
      digitalWrite(scheduler.outPort, HIGH);
    }
    else
    {
      digitalWrite(scheduler.outPort, LOW);
    }
  });
}
